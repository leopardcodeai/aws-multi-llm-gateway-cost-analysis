...
from src.auth.auth import hash_api_key, generate_api_key, create_tenant, get_tenant_by_key, check_quota, record_usage, is_model_allowed
...
class TestAuth:
    ...
    async def test_is_model_allowed_auto(self):
        ...
        assert is_model_allowed(tenant, "gpt-4o") is True
        assert is_model_allowed(tenant, "any-model") is True
    ...
    async def test_is_model_allowed_specific(self):
        ...
        assert is_model_allowed(tenant, "gpt-4o-mini") is True
        assert is_model_allowed(tenant, "gpt-4o") is False